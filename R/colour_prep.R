#' @title Prepare a Colour Palette for Data Columns
#' @name colour_prep
#'
#' @description `colour_prep` works in tandem with `colours_pal`. It is a helper function that prepares a colour
...
#'
#' When a political colour palette is requested from `colour_pal` (prefix "pol"), the function will attempt to assign
...
#' @param pal_name The name of the colour palette to use. The default value is `"catExtended"`,
#'   which refers to an extended categorical colour palette. Other palettes from `colour_pal` can be specified as needed.
#'
#' @return A named list of colours, where each name corresponds to a unique value found in the
#'   specified `columns` of the `data` frame, and each colour is a hexadecimal colour code from the specified
...
#'
#' @export
colour_prep <- function(data, columns, pal_name = "catExtended") {
