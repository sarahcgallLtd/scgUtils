...
test_that("parameters return correct error", {
  expect_error(plot_bigfive("data"), "A data frame is required to be parsed through this function.")

  expect_error(plot_bigfive(df, big_five = c("column1","column2")), "`big_five` variable must be a column in `data`.")
  #expect_error(plot_bigfive(df, big_five = c("gender","partyId")), "`big_five` must be numeric (0-100).")

  expect_error(plot_bigfive(df, group = "column1"), "`group` variable must be a column in `data`.")

  expect_error(plot_bigfive(df, weight = "column1"), "`weight` variable must be a column in `data`.")
  expect_error(plot_bigfive(df, weight = "gender"), "`weight` must be numeric.")
})
