...
# ==============================================================#
  ...
  expect_error(plot_likert(df, vars, group = "gender"),
               "When `group` is specified, `vars` should contain only a single variable.")
...
test_that("function return correct plot for stacked bar chart", {
  p <- plot_likert(df, vars, weight = "wt", varLevels = varLevels_list,
            NET = TRUE, order_by = "left", title = "Un*favourability",
            subtitle = "Political Parties", legend = "top")

  expect_equal(p$guides$fill$nrow, 1)
  expect_equal(p$labels$title, "Un*favourability")
