...
```{r parliament2, dpi=300, out.width = '90%', fig.height = 5}
# Prepare Data
...
### Bar Charts
The `plot_bars()` function provides a clear and concise presentation of categorical data distributions.
<br>
<br>

##### Basic Bar Charts
At its core, `plot_bars()` simplifies the process of generating bar charts for survey data. It calculates frequencies and percentages, adjusting for weights where necessary, and presents this information in an easy-to-understand format.

```{r barchart1, dpi=300, out.width = '100%', fig.height = 4}
# Prepare colours
colours <- colour_prep(survey_df,
                       columns = "generalElectionVote",
                       pal_name = "polUK")
colours$`Brexit Party/Reform UK` <- "#00b1da" # failed to match name

# Plot
plot_bars(survey_df,
          yVar = "generalElectionVote",
          colours = colours,
...
##### Grouped Bar Charts
`plot_bars()` can also group data, allowing for comparisons across demographic segments or other categorical distinctions. This functionality is particularly useful for identifying trends or differences in response patterns among various groups.
```{r barchart2, dpi=300, out.width = '100%', fig.height = 8}
# Prepare colours
colours <- colour_prep(survey_df,
                       columns = "bestOnMII",
                       pal_name = "polUK")
...
plot_bars(survey_df,
          ...
          round_decimals = 0,
          colour = colours,
          title = get_question(survey_df, "bestOnMII"),
          subtitle = paste0("by ", get_question(survey_df, "mii"))
)
```
<br>
