...
# Initialise an empty tibble (data frame) with 1000 rows
binary_df <- tibble(row = 1:1000) %>% select(-row)

# Add columns Q1a to Q1j with varying proportions of "Yes" and "No"
for (col in letters[1:10]) {
  col_name <- paste0("Q1", col)
  yes_proportion <- runif(1) # Random proportion for "Yes"
  binary_df[[col_name]] <- factor(sample(c("Yes", "No"), 1000, replace = TRUE, prob = c(yes_proportion, 1 - yes_proportion)),
                                  levels = c("Yes", "No"))
}

# Add a "Weight" column with values between 0.5 and 2.0
binary_df$Weight <- runif(1000, min = 0.5, max = 2.0)

# Add a "Gender" column with approximately equal "Male" and "Female" values
binary_df$Gender <- factor(sample(c("Male", "Female"), 1000, replace = TRUE),
...
plot_waffle(waffle_df,
            group = "Category",
            values = "Count",
            isolateVar = "A" # isolate a single plot
)
```
<br>
