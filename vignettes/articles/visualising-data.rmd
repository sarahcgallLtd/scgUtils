...
```{r parliament2, dpi=300, out.width = '95%', fig.height = 5}
# Prepare Data
...
### Likert Scales
Likert scale responses provide nuanced insights into survey data. The `plot_likert()` functions contain three options for visualisations, including:
...
##### Basic Chart with One Question
The optional `varLevels` argument controls the order from left to right of the response variables within `vars`.
```{r likert1, dpi=300, out.width = '100%', fig.height = 4, eval = FALSE}
# Plot with unweighted data and vars on the y-axis
plot_likert(survey_df,
            vars = "pidWeThey",
            varLevels = c("Strongly disagree", "Disagree",
                          "Don't know", "Agree", "Strongly agree") # optional to order Levels from left to right
)
```
##### Enhancing the Chart with a Group and NET
By changing the `varLevels` to a list, allows you to add additional controls, including adding the NET value ("Net Promoter Score") which subtracts the right from the left within the list. You can also opt to add colours using the `colour_pal()` function.
...
# Plot with weighted data and group on the y-axis
plot_likert(survey_df,
            vars = "pidWeThey",
            group = "partyId",
            weight = "wt",
            varLevels = varLevels,
            title = "When I speak about this party, I usually say 'we' instead of 'they'", # Add title
            subtitle = "by Party Identification",
            colours = colours, # add colours
            NET = TRUE, # Add NET score (right - left)
            legend = "bottom" # Move legend to bottom

) +
  ggplot2::geom_vline(xintercept = 50, colour = "white") # add vertical line for readability
```

##### Basic Chart with Many Questions
```{r likert3, dpi=300, out.width = '100%', fig.height = 5}
...
# Prepare varLevels (NB these must match all columns contained within vars
...
##### Adding a Group to the Many Question Chart
```{r likert4, dpi=300, out.width = '100%', fig.height = 7}
